...
- Read repo-root `AGENTS.md` and `docs/CTO_RULES.md` before starting.
...
- RUN1 and RUN2 are internal phases, not stop-points.
- Do not advance to the next story until the current story is `DONE` or `BLOCKED`.
- If context pressure rises on an unfinished story, use same-story handoff to a fresh session instead of normal compaction.
...
- Universal CTO intake: applicable facets + candidate P1/P2/P3 risks
...
Follow repo rules:
- If `scripts/quality_gates.py` exists, run `python3 scripts/quality_gates.py` as the primary gate.
- Runtime code changes: run the smallest relevant lint/test commands for the changed path (for example `ruff check .`, `ruff format --check .`, and targeted `pytest`).
...
r use destructive git commands unless explicitly instructed.
